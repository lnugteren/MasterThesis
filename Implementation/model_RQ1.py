...
def too_large_to_choose(path, chunk_size, meta, cols):
    """
    ...
    with open(path, 'r') as f:
        ...
        while line:
            ...
            with open(f"Data/csvs_{category}/file{chunk}.csv", 'w') as output_f:
                while line:
...
def choose_data(data):
    ...
    if zipped:
        ...
        metadata = load_data_gz(f'Data/meta_{data}.json.gz')

        df_review = reviewdata[['overall', 'vote', 'asin', 'reviewText', 'summary', 'unixReviewTime', 'reviewerID']]
        df_meta = metadata[['asin', 'category', 'title', 'description', 'feature', 'main_cat', 'date']]
    else:
        if category == "Electronics" or category == "Home_and_Kitchen":
            df_meta = too_large_to_choose('a', 1, True, "a,b")
            df_meta.columns = ['asin','date']
        else:
            metadata = load_data(f'Data/meta_{data}.json')
            df_meta = metadata[['asin', 'date']]

        df_review = too_large_to_choose(f'Data/{data}.json', 1000000, False,
                                        "overall,vote,asin,reviewText,summary,unixReviewTime,reviewerID")
...
def prepare_df(review_data, meta_data, asin_match):
    """
...
def get_adjacency_matrix(df, col_name):
    ...
    # loop over all summaries per summary to find the cos_sim, no self-loops, not the same revievwer, the 3 highest sim get an edge, if no similarities, no edges
...
def check_majority(cores, neighbor_dict, break_tie):
    ...
    if prnt:
        print("\nfinal 2D core nodes after majority:\n", dups)
...
def graph_peeling(core2D, core, gr):
    ...
    for node2D in core2D:
        ...
        if prnt:
            print("node: {}, pred: {}".format(node2D, pred))
    ...
    if prnt:
        print("\nfinal core nodes after nodes with edges from core:\n", temp_core)
...
def prepare_all_products():
    ...
    if prnt:
        print("Length df_review: ", len(df_review))
        print("Length df_meta: ", len(df_meta))
        print("\n")
    ...
    if prnt:
        print(df_review_pros.sort_values('vote', ascending=False).head(2))
        print("\n")
    ...
    if prnt:
        print("amount of products without reviews: {}".format(df_meta_with_amounts['#reviews'].isna().sum()))
        print("amount of products with reviews: {}".format(
            len(df_meta_with_amounts.loc[df_meta_with_amounts['#reviews'] >= 1])))
        print(df_meta_with_amounts.sort_values('#reviews', ascending=False).head(2))
...
def find_correct_product(reviews_df, metas_df, nr_rev, f):
    ...
    if len(possible_dfs) > 0:
        ...
        reviews, product = prepare_df(reviews_df, metas_df, asin_id)
        if prnt:
            print(product.T)
        ...
        one_product_reviews = one_product_reviews.drop(
            ['unixReviewTime', 'date_x', 'elapsedDays', 'helpfulness', '#reviews', 'date_y'], 1)
...
def prepare_one_product(df, f):
    ...
    df['clean_reviewText'] = [clean_text(text) for text in df['reviewText']]
    df['clean_summary'] = [clean_text(text) for text in df['summary']]
    ...
    if prnt:
        print("Total:",
              df['clean_summary'].apply(lambda x: pd.value_counts(x.split(" "))).sum(axis=1).sum())
        print(df['clean_summary'].apply(lambda x: pd.value_counts(x.split(" "))).sum(axis=1))
    ...
    for index in range(len(df)):
        ...
        if count >= n_neighbor:
            ...
            if index >= parallel_threshold:
                ...
                mat = get_adjacency_matrix(df[:index + 1], break_tie, 'clean_summary')
                if prnt:
                    print(mat)
            ...
            if prnt:
                print("final 2D core nodes:", final_2Dcore_nodes)
                print("final core nodes:", reduced_cores)
            ...
            if len(resulting_list) > 0:
                ...
                if len(resulting_list) >= parallel_threshold:
                    ...
                    mat_sen = get_adjacency_matrix(sentence_df.reset_index(), break_tie, 'clean_reviewText')
                ...
                if prnt:
                    show_graph_with_labels(gr2_sen)
                ...
                if prnt:
                    print("\tfinal 2D core nodes:", final_2Dcore_nodes_sen)
                    print("\tfinal core nodes:", reduced_cores_sen)
                ...
                for id_c2d, core2D in enumerate(final_2Dcore_nodes_sen):
                    ...
                    if index in core2D:
                        ...
                        if prnt:
                            print("\t\tcore nodes with direct edges to non-core nodes", peeled_2Dcore_node)
                        ...
                        if index in peeled_2Dcore_node:
                            ...
                            relevance_model.extend(
                                review for review in salient_reviews['clean_reviewText'] if
                                review not in relevance_model)

                            # get relevance of candidate review
                            candidate_review = df.iloc[index]['clean_reviewText']
                            word_vector = Counter(words for review in relevance_model for words in WORD.findall(review))
                            ...
                            relevance_model.append(candidate_review)
...
def start_model():
    ...
    with open(f'{path2}/results.txt', 'a') as f:
        ...
        if len(one_product_reviews) > 0:
            print("Chosen product: {} has {} reviews - {}, it has {} reviews".format(one_product_reviews['asin'].iloc[0], nr_rev,
                                                                  (nr_rev == len(one_product_reviews)), len(one_product_reviews)), file=f)

            one_product_reviews[['ranking', 'overall', 'vote', 'reviewText', 'summary']].sort_values('ranking').to_csv(
            ...
            # apply the algorithm
            cluster_model(final_product, f, nr_rev)
...
if __name__ == '__main__':
    ...
    # categories = ["Home_and_Kitchen", "Movies_and_TV", "Video_Games", "Electronics", "CDs_and_Vinyl"]

...
def prepare_df(review_data, meta_data, asin_match, f):
    ...
    # if test 4, calculate readability before it's further processed
    if test == "test4" or test == "testall":
        reviews['readability'] = [flesch_readability(str(text)) for text in reviews['reviewText']]
        print(
            f"\nReadability scores: {reviews['readability'].tolist()}, on average: {round(reviews['readability'].mean(), 4)}", file=f)
        reviews = reviews[reviews.readability <= 50]
        print(
            f"Readability scores after: {reviews['readability'].tolist()}, on average after: {round(reviews['readability'].mean(), 4)}", file=f)

    print("Shape after: ", reviews.shape, file=f)
...
def clean_text_with_sw(text):
    """
    Function that cleans the review text data but keeps the stop words (test 3)
    :param text: review
    :return: cleaned review
    """
    text_tokens = word_tokenize(text.lower())
...
def count_words(df, f):
    """
    Function that counts the stop words in the summaries and the review texts
    :param df: data
    :param f: file for writing output
    """
    # initialise
    ...
    if test == "test3" or test == "testall":
        # count stopwords and percentage of review text
        for text in df['clean_reviewText']:
            text_tokens = word_tokenize(text.lower())
            count = 0
            for word in text_tokens:
                if word in stopwords:
                    count += 1
            count_review.append(f"{count}/{len(text_tokens)}")
            avg_review.append(count/len(text_tokens))
        # count stopwords and percentage of summary
        for text in df['clean_summary']:
            text_tokens = word_tokenize(text.lower())
            count = 0
            for word in text_tokens:
                if word in stopwords:
                    count += 1
            count_summary.append(f"{count}/{len(text_tokens)}")
            avg_summary.append(count/len(text_tokens))
        # output the results into the results file
        print(f"\nNr of stopwords in review: {count_review}, on average: {round(np.mean(avg_review),4)}", file=f)
        print(f"Nr of stopwords in summary: {count_summary}, on average: {round(np.mean(avg_summary),4)}\n", file=f)
    else:
        pass
...
def flesch_readability(text):
    """
    Function that calculates the flesch readability ease of a text
    :param text:
...
def find_correct_product(reviews_df, metas_df, nr_rev, f):
    ...
    if len(possible_dfs) > 0:
        ...
        if nr_rev % 10 > 0 and test != "test4" and test != "testall":
...
def prepare_one_product(df, f):
    ...
    """
    # clean the reviews and summaries but include stop words (test 3)
    if test == "test3" or test == "testall":
        df['clean_reviewText'] = [clean_text_with_sw(str(text), f) for text in df['reviewText']]
        df['clean_summary'] = [clean_text_with_sw(str(text), f) for text in df['summary']]
        count_words(df, f)
    else:
        # clean the reviews and summaries
    ...
    # filter reviews on review length (test 1)
    if test == "test1" or test == "testall":
        df["review_length"] = [len(rev.split()) for rev in df['clean_reviewText']]
        print(f"\nRemoved indexes: {df[df.review_length > review_length].index.tolist()}", file=f)
        df = df[df.review_length <= review_length]
        print(f"New length of reviews: {len(df)}\n", file=f)

    # filter reviews on summary length (test 2)
    if test == "test2" or test == "testall":
        summary_length = 4
        df["summary_length"] = [len(rev.split()) for rev in df['clean_summary']]
        print(f"\nRemoved indexes: {df[df.summary_length > summary_length].index.tolist()}", file=f)
        df = df[df.summary_length <= summary_length]
        print(f"New length of reviews: {len(df)}\n", file=f)
    ...
    for nr_rev in nr_revs:
        ...
        with open(f'{path2}/results.txt', 'a') as f:
            ...
            if len(one_product_reviews) > 0:
                ...
                if test == "test4":
                    cols = ['ranking', 'overall', 'vote', 'readability', 'reviewText', 'summary']
                else:
                    cols = ['ranking', 'overall', 'vote', 'reviewText', 'summary']
                one_product_reviews[cols].sort_values('ranking').to_csv(
...
if __name__ == '__main__':
    ...
    # categories = ["Sports_and_Outdoors", "Electronics", "Home_and_Kitchen", "Movies_and_TV", "Office_Products"]
    # tests = ["test1", "test2", "test3", "test4", "testall"]
    ...
    if category == "Movies_and_TV":
        ...
        # for test in tests:
    path = f'Results/RQ2/{test}/{category}'
    try:
        os.makedirs(path)
    except OSError:
